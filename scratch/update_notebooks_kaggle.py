        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "### 🚀 Hỗ trợ đa nền tảng: **Kaggle GPU** | **Google Colab** | **Local PC / VS Code**\n",
                    "\n",
                    "Notebook này tự động nhận diện môi trường (Kaggle / Colab / Local), quét ảnh keyframe, trích xuất text tiếng Việt bằng **PaddleOCR**, và lưu file kết quả **`ocr_results.json`** đúng 100% schema của Backend & Elasticsearch (`aic_ocr`).\n",
                    "\n",
                    "---"
                ]
            },
            {
                ...
                "source": [
                    ...
                    "# Cố định numpy < 2.0 để tránh xung đột ABI trên Kaggle / Colab GPU\n",
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## ⚙️ 2. Tự động Nhận diện Môi trường (Kaggle / Colab / Local) & Cấu hình Đường dẫn"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "\n",
                    "print(f\"🔍 Môi trường phát hiện: {'KAGGLE' if IS_KAGGLE else ('COLAB' if IS_COLAB else 'LOCAL / VS CODE')}\")\n",
                    "\n",
                    "if IS_KAGGLE:\n",
                    "    OUTPUT_JSON = Path(\"/kaggle/working/ocr_results.json\")\n",
                    "    CHECKPOINT_JSON = Path(\"/kaggle/working/ocr_results_checkpoint.json\")\n",
                    "    \n",
                    "    # Tự động tìm thư mục keyframes trong /kaggle/input/\n",
                    "    KEYFRAMES_DIR = None\n",
                    "    possible_kf = glob.glob(\"/kaggle/input/**/keyframes*\", recursive=True) + glob.glob(\"/kaggle/input/**/Keyframes*\", recursive=True)\n",
                    "    if possible_kf:\n",
                    "        KEYFRAMES_DIR = Path(possible_kf[0])\n",
                    "    else:\n",
                    "        # Nếu người dùng add dataset với tên bất kỳ\n",
                    "        subdirs = [Path(\"/kaggle/input\") / d for d in os.listdir(\"/kaggle/input\")] if os.path.exists(\"/kaggle/input\") else []\n",
                    "        KEYFRAMES_DIR = subdirs[0] if subdirs else Path(\"/kaggle/input/your-keyframes-dataset/\")\n",
                    "        \n",
                    "    # Tự động tìm metadata_clip.json trong /kaggle/input/\n",
                    "    METADATA_PATH = None\n",
                    "    possible_meta = glob.glob(\"/kaggle/input/**/metadata_clip.json\", recursive=True)\n",
                    "    if possible_meta:\n",
                    "        METADATA_PATH = Path(possible_meta[0])\n",
                    "        \n",
                    "    # Tự động tìm map-keyframes trong /kaggle/input/\n",
                    "    MAP_KEYFRAMES_DIR = None\n",
                    "    possible_map = glob.glob(\"/kaggle/input/**/map-keyframes*\", recursive=True)\n",
                    "    if possible_map:\n",
                    "        MAP_KEYFRAMES_DIR = Path(possible_map[0])\n",
                    "\n",
                    "elif IS_COLAB:\n",
                    "    try:\n",
                    ...
                    "    KEYFRAMES_DIR = Path(\"/content/drive/MyDrive/AIC_Data/Keyframes/keyframes_L21_onwards\")\n",
                    "    METADATA_PATH = Path(\"/content/drive/MyDrive/AIC/Backend/Back-End/src/dict/metadata_clip.json\")\n",
                    "    MAP_KEYFRAMES_DIR = Path(\"/content/drive/MyDrive/AIC/Backend/Back-End/src/dict/map-keyframes\")\n",
                    "\n",
                    "else: # LOCAL PC\n",
                    "    try:\n",
                    ...
                    "    except Exception:\n",
                    "        pass\n",
                    "    PROJECT_ROOT = Path(__file__).resolve().parent.parent if '__file__' in locals() else Path(os.getcwd())\n",
                    "    OUTPUT_JSON = PROJECT_ROOT / \"src\" / \"dict\" / \"ocr_results.json\"\n",
                    "    CHECKPOINT_JSON = PROJECT_ROOT / \"src\" / \"dict\" / \"ocr_results_checkpoint.json\"\n",
                    "    KEYFRAMES_DIR = Path(os.getenv(\"KEYFRAMES_ROOT\", \"D:/AIC_Data/Keyframes/keyframes_L21_onwards\"))\n",
                    "    METADATA_PATH = PROJECT_ROOT / \"src\" / \"dict\" / \"metadata_clip.json\"\n",
                    "    MAP_KEYFRAMES_DIR = PROJECT_ROOT / \"src\" / \"dict\" / \"map-keyframes\"\n",
                    "\n",
                    "print(f\"\\n📁 ĐƯỜNG DẪN CẤU HÌNH:\")\n",
                    "print(f\"- Keyframes Dir:   {KEYFRAMES_DIR} (Tồn tại: {KEYFRAMES_DIR.exists() if KEYFRAMES_DIR else False})\")\n",
                    "print(f\"- Metadata Path:   {METADATA_PATH} (Tồn tại: {METADATA_PATH.exists() if METADATA_PATH else False})\")\n",
                    "print(f\"- Map Keyframes:   {MAP_KEYFRAMES_DIR} (Tồn tại: {MAP_KEYFRAMES_DIR.exists() if MAP_KEYFRAMES_DIR else False})\")\n",
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## 🔍 3. Xây dựng Danh sách Frame cần trích xuất OCR"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "        print(f\"❌ Không tìm thấy thư mục keyframes tại: {keyframes_dir}\")\n",
                    "        return tasks\n",
                    "\n",
                    "    # Ưu tiên 1: Đọc từ metadata_clip.json nếu có\n",
                    "    if metadata_path and os.path.exists(metadata_path):\n",
                    "        print(f\"Nạp từ metadata file: {metadata_path}\")\n",
                    ...
                    "                })\n",
                    "        if tasks:\n",
                    "            print(f\"-> Khớp được {len(tasks)} ảnh trên disk từ metadata.\")\n",
                    "            return tasks\n",
                    "\n",
                    "    # Ưu tiên 2: Quét trực tiếp thư mục Keyframes nếu không có metadata_clip.json\n",
                    ...
                    "print(f\"Tổng số frame chuẩn bị trích xuất: {len(tasks)}\")"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 🤖 4. Khởi tạo PaddleOCR Model (Tiếng Việt)"
                ]
            },
            {
                "cell_type": "code",
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## ⚡ 5. Thực hiện Trích xuất OCR & Checkpoint Auto-Save"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "\n",
                    "# Lưu file kết quả chính thức\n",
                    "final_results = list(results_dict.values())\n",
                    "os.makedirs(os.path.dirname(OUTPUT_JSON), exist_ok=True)\n",
                    "with open(OUTPUT_JSON, 'w', encoding='utf-8') as f:\n",
                    "    json.dump(final_results, f, ensure_ascii=False, indent=2)\n",
                    "\n",
                    "print(f\"\\n✅ HOÀN THÀNH! Đã lưu {len(final_results)} bản ghi OCR vào: {OUTPUT_JSON}\")"
                ]
            },
            {
                "cell_type": "markdown",
                "metadata": {},
                "source": [
                    "## 📊 6. Kiểm tra Kết quả & Hướng dẫn Tải về"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "        data = json.load(f)\n",
                    "    print(f\"Tổng số frame có text OCR: {len(data)}\")\n",
                    "    if data:\n",
                    "        print(\"\\n--- 3 MẪU ĐẦU TIÊN ---\")\n",
                    ...
                    "    if IS_KAGGLE:\n",
                    "        print(\"👉 Bạn có thể tải file `ocr_results.json` ở tab 'Output' bên phải màn hình Kaggle.\")\n",
                    "        print(\"👉 Sau khi tải về, copy vào thư mục `Backend/Back-End/src/dict/ocr_results.json`.\")"
                ]
            }
        ],
        "metadata": {
        ...
        "cells": [
            {
                ...
                "source": [
                    ...
                    "### 🚀 Hỗ trợ đa nền tảng: **Kaggle GPU** | **Google Colab** | **Local PC / VS Code**\n",
                    "\n",
                    "Notebook này tự động nhận diện môi trường (Kaggle / Colab / Local), trích xuất giọng nói tiếng Việt bằng **faster-whisper (Large-v3-Turbo)**, tự động căn chỉnh với keyframe gần nhất và lưu file kết quả **`asr_results.json`** đúng 100% schema Elasticsearch `aic_asr` của Backend.\n",
                    "\n",
                    "---"
                ]
            },
            ...
            },
            {
                ...
                "metadata": {},
                "source": [
                    "## ⚙️ 2. Tự động Nhận diện Môi trường & Cấu hình Đường dẫn"
                ]
            },
            {
                "cell_type": "code",
                ...
                "source": [
                    ...
                    "    \n",
                    "    # Tự động tìm thư mục Videos/Audio trong /kaggle/input/\n",
                    "    MEDIA_DIR = None\n",
                    ...
                    "        \n",
                    "    # Tự động tìm metadata_clip.json hoặc map-keyframes\n",
                    "    METADATA_PATH = None\n",
                    "    possible_meta = glob.glob(\"/kaggle/input/**/metadata_clip.json\", recursive=True)\n",
                    "    if possible_meta:\n",
                    "        METADATA_PATH = Path(possible_meta[0])\n",
                    ...
                    "    MEDIA_DIR = Path(\"/content/drive/MyDrive/AIC_Data/Videos\")\n",
                    "    METADATA_PATH = Path(\"/content/drive/MyDrive/AIC/Backend/Back-End/src/dict/metadata_clip.json\")\n",
                    "    MAP_KEYFRAMES_DIR = Path(\"/content/drive/MyDrive/AIC/Backend/Back-End/src/dict/map-keyframes\")\n",
                    "\n",
                    "else: # LOCAL PC\n",
                    ...
                    "        pass\n",
                    "    PROJECT_ROOT = Path(__file__).resolve().parent.parent if '__file__' in locals() else Path(os.getcwd())\n",
                    "    OUTPUT_JSON = PROJECT_ROOT / \"src\" / \"dict\" / \"asr_results.json\"\n",
                    "    CHECKPOINT_JSON = PROJECT_ROOT / \"src\" / \"dict\" / \"asr_results_checkpoint.json\"\n",
                    "    MEDIA_DIR = Path(\"D:/AIC_Data/Videos\") if os.path.exists(\"D:/AIC_Data/Videos\") else PROJECT_ROOT / \"src\" / \"data\" / \"Videos\"\n",
                    "    METADATA_PATH = PROJECT_ROOT / \"src\" / \"dict\" / \"metadata_clip.json\"\n",
                    "    MAP_KEYFRAMES_DIR = PROJECT_ROOT / \"src\" / \"dict\" / \"map-keyframes\"\n",
                    "\n",
                    "print(f\"\\n📁 ĐƯỜNG DẪN CẤU HÌNH:\")\n",
                    "print(f\"- Media Dir:       {MEDIA_DIR} (Tồn tại: {MEDIA_DIR.exists() if MEDIA_DIR else False})\")\n",
                    "print(f\"- Metadata Path:   {METADATA_PATH} (Tồn tại: {METADATA_PATH.exists() if METADATA_PATH else False})\")\n",
                    "print(f\"- Map Keyframes:   {MAP_KEYFRAMES_DIR} (Tồn tại: {MAP_KEYFRAMES_DIR.exists() if MAP_KEYFRAMES_DIR else False})\")\n",
                    "print(f\"- File Output:     {OUTPUT_JSON}\")"
                ]
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    \n",
                    "    if metadata_path and os.path.exists(metadata_path):\n",
                    "        print(f\"Nạp keyframes từ metadata: {metadata_path}\")\n",
                    "        with open(metadata_path, 'r', encoding='utf-8') as f:\n",
                    "            meta = json.load(f)\n",
                    "        for fid_str, info in meta.items():\n",
                    ...
                    "\n",
                    "    if map_keyframes_dir and os.path.exists(map_keyframes_dir):\n",
                    "        print(f\"Nạp keyframes từ thư mục CSV: {map_keyframes_dir}\")\n",
...
print("Successfully updated notebooks with intelligent multi-platform auto-detection (Kaggle / Colab / Local)!")
