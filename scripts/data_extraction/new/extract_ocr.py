...
Extracts text from video keyframes using PaddleOCR and saves the results
...
# Try importing paddleocr, but allow the script to fail gracefully if not installed
try:
...
def main():
    ...
    if PaddleOCR is None:
        logger.error("PaddleOCR is not installed. Please run `pip install paddleocr paddlepaddle`.")
        sys.exit(1)
    ...
    # Initialize PaddleOCR reader (Vietnamese and English)
    logger.info("Initializing PaddleOCR reader (vi)... This may download model weights.")
    # PaddleOCR uses GPU by default if paddlepaddle-gpu    # Initialize PaddleOCR (updated for newer versions)
    import logging
