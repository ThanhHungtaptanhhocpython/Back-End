...
def _module_check(module_name: str) -> ReadinessCheck:
    if module_name in sys.modules:
        # Already imported in this process -> definitely installed. (find_spec
        # raises ValueError "<mod>.__spec__ is not set" for an already-loaded
        # module whose spec was cleared, e.g. after a reload elsewhere.)
        found = True
    else:
        try:
...
def _jina_model_is_local(settings: Settings) -> bool:
    return Path(str(settings.jina_model_path)).expanduser().exists()


def _jina_checks(settings: Settings) -> list[ReadinessCheck]:
    from src.services.retrieval_backend import JINA_CLIP_V2, active_backend

    checks: list[ReadinessCheck] = []
    # Canonical: RETRIEVAL_BACKEND (VISUAL_RETRIEVER is a deprecated alias that
    # settings normalises into it), with CLOUD_ASSETS_ENABLED forcing Jina.
    selected = active_backend(settings)
    checks.append(
        _check(
            "retrieval_backend",
            selected == JINA_CLIP_V2,
            f"active backend is {selected!r}; competition target is {JINA_CLIP_V2!r} "
            f"(RETRIEVAL_BACKEND / CLOUD_ASSETS_ENABLED).",
            category="retrieval",
        )
    )
    ...
    # Optional compatibility input -- canonical/cloud metadata is enough on its
    # own, so an unset path is a pass, not a failure.
    if settings.jina_video_metadata_path is None:
        checks.append(
            ReadinessCheck(
                "jina_video_metadata",
                S_PASS,
                "optional; not configured (canonical/cloud metadata is authoritative).",
                "assets",
            )
        )
    else:
        checks.append(_file_check(settings.jina_video_metadata_path, "jina_video_metadata"))
    ...
    if model_local:
        checks.append(
            ReadinessCheck(
                ...
                S_PASS,
                f"using local model path: {settings.jina_model_path}",
                "retrieval",
            )
        )
    else:
...
def _cloud_checks(settings: Settings) -> list[ReadinessCheck]:
    ...
    # The canonical default is 'hcmai-assets.json'. A deliberate per-dataset
    # override is fine; only a blank key is a failure.
    checks.append(
        _check(
            "cloud_manifest_key",
            bool(manifest_key),
            f"CLOUD_ASSETS_MANIFEST_KEY={manifest_key!r}"
            + ("" if manifest_key == "hcmai-assets.json" else " (override of the 'hcmai-assets.json' default)"),
            category="cloud",
        )
    )
