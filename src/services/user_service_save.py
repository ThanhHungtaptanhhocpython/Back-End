...
scene_id2info_path = os.path.join(SRC_DIR, 'dict', 'scene_id2info.json')
...
with open(scene_id2info_path, 'r') as f:
...
def getImageDataSingleTextSearch(clip, clipv2, text_query, k):
    ...
    for img_id in list_ids:
        ...
        frame_number = KeyframesMapper.get(f"{folder_key}_{video_key}", {}).get(frame_key, None)
        video_id = f"{folder_key}_{video_key}"
        fps = FpsDict.get(video_id, 25)
